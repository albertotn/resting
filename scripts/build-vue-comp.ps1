...
& "$CLI_PATH/vue-cli-service.ps1" build --target lib --formats umd --dest $DIST_FOLDER --no-clean --name authentication-panel  "$COMPONENTS_FOLDER/AuthenticationPanel.vue"
& "$CLI_PATH/vue-cli-service.ps1" build --target lib --formats umd --dest $DIST_FOLDER --no-clean --name header  "$COMPONENTS_FOLDER/Header.vue"
